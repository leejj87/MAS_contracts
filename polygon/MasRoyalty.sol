...
contract royalty is Ownable, ERC2981PerTokenRoyalties {
    ...
    modifier creatorOnly(uint256 _tokenId) {
        require(
            nftAddress.getCreators(_tokenId) == msg.sender,
            "ERC1155Tradable#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    event royalty_record(
        address who_triggered,
        uint256 tokenId,
        uint16 royaltySetup
    );
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return ERC2981Base.supportsInterface(interfaceId);
    }
    ...
    function setRoyalty(uint256 _tokenId, uint16 _royalties)
        public
